...
(async () => {
  ...
  const gatedDeepLink = await fetch(gatedBase + "/?tool=frequency", { redirect: "manual" });
  assert.equal(gatedDeepLink.status, 302);
  assert.match(gatedDeepLink.headers.get("location"), /\/login\.html\?next=/);
  assert.match(decodeURIComponent(gatedDeepLink.headers.get("location")), /tool=frequency/);
